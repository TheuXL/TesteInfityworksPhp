<template>
  <div>
    <div class="mb-4 flex items-center justify-between">
      ...
      <router-link to="/admin/enrollments/create"><AppButton>Nova matrícula</AppButton></router-link>
    </div>
    <div class="mb-3">
      <label class="flex items-center gap-2 text-sm text-slate-600">
        ...
        >
          ...
          <option value="name_asc">A–Z (aluno)</option>
          <option value="name_desc">Z–A (aluno)</option>
        </select>
      </label>
    </div>
    ...
    <div v-else class="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
      <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
        <thead class="bg-slate-50">
          <tr>
            <th class="px-4 py-3 font-semibold text-slate-700">Aluno</th>
        ...
        <tbody class="divide-y divide-slate-200">
          <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3">
              <router-link :to="`/admin/enrollments/${item.id}/edit`" class="text-blue-600 hover:underline">Editar</router-link>
...
async function fetch() {
  loading.value = true;
  try {
    const { data } = await EnrollmentService.getAll({ sort: sort.value, per_page: 5000 });
...
async function remove(item) {
  ...
  await EnrollmentService.delete(item.id);
