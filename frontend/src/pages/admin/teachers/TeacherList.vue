<template>
  <div>
    <div class="mb-4 flex items-center justify-between">
      <h2 class="text-2xl font-bold text-slate-900">Professores</h2>
      <router-link to="/admin/teachers/create"><AppButton>Novo professor</AppButton></router-link>
    </div>
    <div class="mb-3">
      ...
      <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-200">
          <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50">
            ...
            <td class="px-4 py-3">
              <router-link :to="`/admin/teachers/${item.id}/edit`" class="text-blue-600 hover:underline">Editar</router-link>
...
async function fetch() {
  loading.value = true;
  try {
    const { data } = await TeacherService.getAll({ sort: sort.value, per_page: 5000 });
...
async function remove(item) {
  if (!confirm(`Excluir professor "${item.name}"?`)) return;
