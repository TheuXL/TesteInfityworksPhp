# **Plataforma Prof. Jubilut – Backend (Laravel API)**

Bem-vindo ao **backend** da Plataforma Prof. Jubilut: uma API REST em **Laravel 10** que centraliza a lógica de negócio, autenticação por papéis (admin e aluno), CRUD de áreas, cursos, professores, disciplinas, alunos e matrículas, relatórios por curso (idade média, aluno mais novo/mais velho) e dados para gráficos no dashboard admin e no dashboard do aluno. O backend foi pensado para ser consumido por um **frontend SPA** (Vue) e por futuros clientes (ex.: app mobile), com **Laravel Sanctum** para autenticação stateful e por token.

O sistema diferencia **administradores** e **alunos** por meio do campo `role` na tabela `users` e de middlewares dedicados (`role.admin` e `role.student`). O cadastro de aluno é público (`POST /register`); o cadastro de admin é restrito à área admin (`POST /api/v1/admin/register`). Toda a persistência é feita em **MySQL**, com tabelas criadas **exclusivamente por migrations** e integridade referencial via foreign keys.
...
1. **🔐 Autenticação e Autorização por Papel:** Login único (`POST /login`, `POST /login/admin`, `POST /login/aluno`) com validação de `role`; rotas `/api/v1/admin/*` exigem `role.admin` e `/api/v1/aluno/*` exigem `role.student`. Cadastro de aluno (público) e de admin (apenas por admin logado). Documentação detalhada em [docs/ROLES-E-AUTH.md](docs/ROLES-E-AUTH.md).

2. **📊 CRUD Completo (Admin):** Áreas, Cursos, Professores, Disciplinas, Alunos e Matrículas (enrollments), com validação via **Form Requests**, respostas em **API Resources** e ordenação/paginação. Listagem de alunos com **filtro por nome e e-mail** (scope `Student::search()`).

3. **📈 Relatórios e Gráficos:** Serviço **ReportService** para relatório por curso (média de idade, aluno mais novo e mais velho, usando Carbon para cálculo de idade). Serviço **StudentChartDataService** para dados de gráficos no dashboard admin (alunos por curso, idade média por curso, alunos por faixa etária, matrículas por curso/mês, alunos por área, disciplinas por curso, etc.) e no dashboard do aluno (meus cursos, minha idade, minhas matrículas).

4. **👤 Área do Aluno:** O aluno acessa apenas **perfil** (GET/PUT `/api/v1/aluno/profile`) e **dashboard** com dados de gráficos; ele altera somente seus próprios dados (nome, e-mail, data de nascimento), garantido pelo middleware e pelo vínculo `user -> student`.
...
6. **🐳 Docker e Ambiente:** Dockerfile (PHP 8.2, Composer, extensões PDO MySQL, etc.) e `docker-entrypoint.sh` que espera o MySQL, roda `migrate` e `db:seed` antes de subir o servidor. Seed de desenvolvimento (admin, áreas, cursos, professores, disciplinas, alunos e matrículas) para testes e demonstração.
...
- **CORS e Sanctum configurados** para SPA (frontend Vue em localhost:5173 ou domínio configurável).
...
graph TB
    subgraph "Cliente"
        A[Frontend Vue SPA<br/>localhost:5173]
        B[Futuro App Mobile]
    end
    ...
    subgraph "API v1 Admin – role.admin"
        ...
        K[apiResource areas, courses, teachers, disciplines, students, enrollments]
    ...
    subgraph "Serviços e Repositórios"
        ...
        T[AreaRepository, CourseRepository, StudentRepository]
    end
...
Detalhes e fluxo frontend/backend estão em **[docs/ROLES-E-AUTH.md](docs/ROLES-E-AUTH.md)**.
...
- **API v1 Admin (`app/Http/Controllers/Api/v1/Admin/`):**  
  ...
  - **DashboardController:** dados do dashboard admin (chart data via StudentChartDataService).  
...
- **Authenticate, EncryptCookies, VerifyCsrfToken, TrimStrings, TrustProxies, PreventRequestsDuringMaintenance, RedirectIfAuthenticated, ValidateSignature:** Uso padrão Laravel para web e API.

Registrados no **Kernel** como `role.admin` e `role.student` no grupo de aliases.

### Form Requests (`app/Http/Requests/`)

Validação dedicada para cada entidade e ação, por exemplo:  
StoreAreaRequest, UpdateAreaRequest; StoreCourseRequest, UpdateCourseRequest; StoreTeacherRequest, UpdateTeacherRequest; StoreDisciplineRequest, UpdateDisciplineRequest; StoreStudentRequest, UpdateStudentRequest; StoreEnrollmentRequest, UpdateEnrollmentRequest; **UpdateProfileRequest** (perfil do aluno: name, email, birth_date). Regras (required, string, email, unique, date, etc.) ficam nos Form Requests, mantendo os controllers enxutos.
...
- **StudentChartDataService:**  
  - **adminChartData():** summary (contagens), students_per_course, average_age_per_course, students_by_age_range, enrollments_per_course, students_per_area, enrollments_per_month, students_per_month, disciplines_per_course (labels + data para Chart.js).  
...
- **Gráficos admin:** Contagens e agregados a partir de repositórios e models (courses com withCount/with, areas com courses e students, enrollments e students por período). Faixas etárias: 0-17, 18-25, 26-35, 36-50, 50+.  
...
- **Docker:** O **Dockerfile** usa imagem `php:8.2-cli`, instala dependências de sistema e extensões, Composer, e copia a aplicação; o **docker-entrypoint.sh** espera o MySQL estar disponível, gera `APP_KEY` se necessário, executa `migrate --force` e `db:seed --force`, depois inicia `php artisan serve --host=0.0.0.0 --port=8000`. Use `.env.docker` como referência para variáveis no container (ex.: `DB_HOST=mysql`). O `docker-compose` na raiz do repositório (fora do backend) pode orquestrar app e MySQL.

Para instruções passo a passo de instalação e seed (incluindo usuário admin padrão), consulte **[INSTALACAO.md](INSTALACAO.md)**.

---

**Resumo:** Este backend implementa a API completa da Plataforma Prof. Jubilut: autenticação por papéis, CRUD de todas as entidades, relatórios por curso, dados para gráficos no admin e no aluno, perfil do aluno e integração com frontend SPA e Docker, com banco MySQL 100% gerenciado por migrations.
